...
class ProductsRequests < Base
    ...
    def post_product(token, payload)
        self.class.post('/produtos', headers: { "Authorization": token, "Content-Type": "application/json" }, body: payload.to_json)
    end

    def put_product(token, id, payload)
        self.class.put("/produtos/#{id}", headers: { "Authorization": token, "Content-Type": "application/json" }, body: payload)
    end
end
